...
for datastore in $(echo "${datastores}" | jq -r '.[] | @base64'); do
  ...
  echo "curl $GS_REST/workspaces/$wsName/datastores/$dsName.json > $EXPORT_PATH/workspaces/$wsName/datastores/$dsName/datastore.json"
  curl $GS_REST/workspaces/$wsName/datastores/$dsName.json > $EXPORT_PATH/workspaces/$wsName/datastores/$dsName/datastore.json
  cat $EXPORT_PATH/workspaces/$wsName/datastores/$dsName/datastore.json | jq .
  echo
